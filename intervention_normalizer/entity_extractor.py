...
def extract_entities(snippet, matcher, exclude_terms, sem_map, is_initial):
    lst = []
    result = matcher.match(snippet, best_match=True, ignore_syntax=False)
    for item in result:
        if is_initial:
            item = sorted(item, key=lambda x: x['similarity'], reverse=True)
        else:
            item = sorted(item, key=lambda x: (len(x['ngram'].split(' ')), x['similarity']), reverse=True)
        selected = item[0]
        ...
        if re.match(f'^({exclude_terms})$', text, re.I):
            ...
            types = selected['semtypes']
            type_full = set()
            for tp in types:
                type_full.add(sem_map[tp])
            selected['semtypes'] = type_full
            lst.append(selected)
...
def remapping(entities, merged, rep, processed, matcher, sem_map, excluded_terms):
    ...
    for key, value in merged.items():
        ...
        result = extract_entities(processed[start: end], matcher, excluded_terms, sem_map, False)
        if len(result) == 0:
